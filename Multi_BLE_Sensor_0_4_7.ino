...
void setup() {
  ...
  current_crank_time = millis();
  current_crank_pulse = 100;
  lowerpulse = 300;
  upperpulse = 500;
  //current_crank_pulse = random(lowerpulse, upperpulse);
  
  // Random complete revolution in faux milliseconds
...
void loop() {
  ...
  if (millis() >= last_crank_time + current_crank_pulse)               // If current millis is greater or equal to previous time millis + faux random millis
     {
     // Calculate crank timings     
    
     //current_crank_pulse = random(lowerpulse, upperpulse);
     //current_crank_pulse = 50;
     // Create new faux random millis as pretend complete crank revolution 
     //current_crank_time = millis();                             // Set current crank time to now in millis
     diff_crank_time = millis() - last_crank_time;    // difference in millis is now in millis - the last crank in millis
     ...
     Serial.println (calc_crank_time);
 ...
 delay(current_crank_pulse);                                                     // Wait x milliseconds for BLE operations to complete, avoiding the ESP32 crashing
}
